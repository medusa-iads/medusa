...
-- Items processed per phase invocation. With 100 items and budget 20,
-- a full cycle completes in 5 refills x 2 ticks = 1 second at 10 Hz.
local CHUNK_BUDGET_TRACKS = 20
local CHUNK_BUDGET_HARM = 15
local CHUNK_BUDGET_BATTERIES = 20
...
function ChunkStep.new(budget)
	return setmetatable({ _queue = Queue(), _budget = budget }, ChunkStep)
end
...
function Medusa.Core.IadsNetwork:new(opts)
	local o = {
		...
		_classifyStep = ChunkStep.new(CHUNK_BUDGET_TRACKS),
		_harmDetectStep = ChunkStep.new(CHUNK_BUDGET_HARM),
		_handoffStep = ChunkStep.new(CHUNK_BUDGET_BATTERIES),
		_deactivationStep = ChunkStep.new(CHUNK_BUDGET_BATTERIES),
...
function Medusa.Core.IadsNetwork:_runPhase()
	...
	if phase == 0 then
		self:_phaseClassify(trackStore, batteryStore, geoGrid, now, maxRange, hpt, MS)
	elseif phase == 1 then
		self:_phaseHarmAndPD(trackStore, batteryStore, geoGrid, now, hpt, MS)
	elseif phase == 2 then
		self:_phaseAssign(trackStore, batteryStore, geoGrid, now, maxRange, hpt, MS)
	elseif phase == 3 then
		self:_phaseMaintain(trackStore, batteryStore, now, hpt, MS)
	elseif phase == 4 then
		self:_phaseEmcon(batteryStore, now, hpt, MS)
	end
...
function Medusa.Core.IadsNetwork:_phaseClassify(trackStore, batteryStore, geoGrid, now, maxRange, hpt, MS)
	...
	local freshCycle = step:fill(trackStore:getAll(_assignBatteryBuffer))
	if freshCycle then
		TC.clearPromotedBuffer()
	end

	for _ = 1, step._budget do
		...
		if promotion then
			Medusa.Services.TrackClassifier._promotedBuffer[#Medusa.Services.TrackClassifier._promotedBuffer + 1] =
				promotion
		end
	...
	if step:isEmpty() and guiltEnabled then
		local tracks = trackStore:getAll(_assignBatteryBuffer)
		TC.flushGuiltByAssociation(tracks, trackStore, now)
	end
...
function Medusa.Core.IadsNetwork:_phaseMaintain(trackStore, batteryStore, now, hpt, MS)
	...
	for _ = 1, handoffStep._budget do
